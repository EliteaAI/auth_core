...
"""Grant Skill publish and icon-upload permissions to roles that already carry a per-project override snapshot."""

revision = "202607161200"
down_revision = "202606241200"
branch_labels = None
...
_ROLES = ("admin", "editor", "super_admin", "system")  # viewer intentionally excluded

ROLE_SKILL_PAIRS = [(role, perm) for role in _ROLES for perm in NEW_PERMISSIONS]
...
def downgrade(module, payload):
    ...
    # Exact inverse of the permission list added above. Deliberately NOT a
    # prefix-wide skills.% delete: that would also remove override rows a
    # project admin granted independently of this migration.
    op.execute(
        sa.text(
            f"""
